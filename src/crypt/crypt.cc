...
std::string AESImpl::Encrypt(const void* input, size_t input_size) {
  FillRandomBytes(iv_, sizeof(iv_));
  mbedtls_cipher_set_iv(&encrypt_ctx_, iv_, sizeof(iv_));
  mbedtls_cipher_reset(&encrypt_ctx_);

  std::string output(sizeof(iv_) + input_size + encrypt_block_size_, 0);
  std::memcpy(&output[0], iv_, sizeof(iv_));

  size_t olen = 0;
  int ret = mbedtls_cipher_update(
      &encrypt_ctx_, reinterpret_cast<const unsigned char*>(input), input_size,
      reinterpret_cast<unsigned char*>(&output[sizeof(iv_)]), &olen);
  if (ret != 0) {
    return "";
  }
  size_t olen2 = 0;
  ret = mbedtls_cipher_finish(
      &encrypt_ctx_,
      reinterpret_cast<unsigned char*>(&output[sizeof(iv_)]) + olen, &olen2);
  if (ret != 0) {
    return "";
  }
  output.resize(sizeof(iv_) + olen + olen2);
  return output;
}

std::string AESImpl::Decrypt(const void* input, size_t input_size) {
  if (input_size < sizeof(iv_)) {
    return "";
  }
  const unsigned char* data = reinterpret_cast<const unsigned char*>(input);
  std::memcpy(iv_, data, sizeof(iv_));
  mbedtls_cipher_set_iv(&decrypt_ctx_, iv_, sizeof(iv_));
  mbedtls_cipher_reset(&decrypt_ctx_);

  size_t cipher_size = input_size - sizeof(iv_);
  std::string output(cipher_size + decrypt_block_size_, 0);
  size_t olen = 0;
  int ret = mbedtls_cipher_update(
      &decrypt_ctx_, data + sizeof(iv_), cipher_size,
      reinterpret_cast<unsigned char*>(&output[0]), &olen);
  if (ret != 0) {
    return "";
  }
  size_t olen2 = 0;
  ret = mbedtls_cipher_finish(
      &decrypt_ctx_, reinterpret_cast<unsigned char*>(&output[0]) + olen,
      &olen2);
  if (ret != 0) {
    return "";
  }
